...
int main(int argc, char *argv[]) {
   ...
   std::ifstream in2;
    ...
    FIFOReplacement fi(num_pages, num_frames);
    start = clock();
    for (std::vector<int>::const_iterator it = large_refs.begin(); it != large_refs.end(); ++it) {
        int page_num = (*it) >> page_offset_bits;
        bool isPageFault = fi.access_page(page_num, 0);
        PageEntry pg = fi.getPageEntry(page_num);
    }
    end = clock();
    fi.print_statistics();
    time = ((double)(end - start))/CLOCKS_PER_SEC;
    cout<<"Elased time:"<<time<<endl;
    ...
    // TODO: print the statistics and run-time
        LIFOReplacement li(num_pages, num_frames);
        start = clock();
        for (std::vector<int>::const_iterator it = large_refs.begin(); it != large_refs.end(); ++it) {
        int page_num = (*it) >> page_offset_bits;
        bool isPageFault = li.access_page(page_num, 0);
        PageEntry pg = li.getPageEntry(page_num);
    ...
    // TODO: print the statistics and run-time
        LRUReplacement lr(num_pages, num_frames);
        start = clock();
        for (std::vector<int>::const_iterator it = large_refs.begin(); it != large_refs.end(); ++it) {
        int page_num = (*it) >> page_offset_bits;
        bool isPageFault = lr.access_page(page_num, 0);
        PageEntry pg = lr.getPageEntry(page_num);
